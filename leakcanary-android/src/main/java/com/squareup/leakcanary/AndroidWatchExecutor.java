...
public final class AndroidWatchExecutor implements WatchExecutor {
    ...
    public void execute(@NonNull Retryable retryable) {
        if (Looper.getMainLooper().getThread() == Thread.currentThread()) { //主线程
            waitForIdle(retryable, 0);//idelHandler 返回false 主线程空闲时间 执行一次
        } else {
            postWaitForIdle(retryable, 0);//如果子线程中 则抛到主线程中
            // 等主线程idel时机时候，抛真正的执行任务到子线程中执行，然后retry时下一次抛到到主线程中idle中进行分发到子线程中执行
        }
    }
