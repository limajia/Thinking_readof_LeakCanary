...
public final class AndroidHeapDumper implements HeapDumper {
    ...
    public File dumpHeap() {
        ...
        try {
            // log tag指定为：heap dump 则可以看到采集路径 到分析结果的打印日志
            // 安卓平台提供的获取堆信息的方法
            Debug.dumpHprofData(heapDumpFile.getAbsolutePath()); //2021-09-18_20-08-12_449.hprof 文件是android下的文件 可以用androidstudio打开，
            //不同于其他的mat工具等的文件 需要android工具 进行转化
            // shallow size 引用的对象本身大小
            // ratained size 删除改对象后，自己及先关的对象删除的大小
            // depth 标识到gc对象的距离 为1 则标识gc对象 直接引用着当前对象
            cancelToast(toast);
